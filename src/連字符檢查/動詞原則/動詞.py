...
def 是環綴起始(字分詞):
    集合 = ('會｜e7', '袂｜be7', '袂｜bue7')
    return 字分詞 in 集合


def 是環綴結尾(字分詞):
    集合 = ('得｜tit4', '得｜tsit4', '得｜lit4', '得｜eh4')
    return 字分詞 in 集合
...
def 是否符合環綴原則(句物件):
    ...
    for 詞位置, 詞物件 in enumerate(詞物件陣列):
        ...
        for 字物件 in 詞的字物件陣列:
            ...
            if 是環綴起始(字分詞):
                print('是環綴起始=', 字分詞)
                環綴起始詞.update({'詞位置': 詞位置, '字陣列': 詞的字物件陣列, '累積字長度': 累積字長度})
            # 遇到可能是環綴結尾的詞
            # if 是環綴結尾 and 前面有找到環綴起始詞 and 環綴起始詞的詞位置 != 現在環綴結尾詞的詞位置
            elif 是環綴結尾(字分詞) and 環綴起始詞 != None and 環綴起始詞['詞位置'] != 詞位置:
                # 偵測到句子有出現環綴，檢查環綴的書寫是否正確
                環綴結尾詞 = 詞的字物件陣列
                # 首字應連寫
                # 會 記-得  1 2-2 錯誤
                # 會 記 得  1 2 3 錯誤
