...
def 是否符合咧原則(句物件):
    ...
    for 詞位置, 詞物件 in enumerate(詞物件陣列):
        ...
        for 字位置, 字物件 in enumerate(詞的字物件陣列):
            ...
            elif 是咧 and not 是咧詞組(詞分詞) and 前半段的詞 and not 是動詞(前半段的詞):
                錯誤行數 = 累積字長度 + 字位置 + 1
                錯誤連字符位置 = '前'
                錯誤訊息陣列.append(('E動詞（二）', 錯誤行數, 錯誤連字符位置))
...
def 是否符合著原則(句物件):
    ...
    for 詞位置, 詞物件 in enumerate(詞物件陣列):
        ...
        for 字位置, 字物件 in enumerate(詞的字物件陣列):
            ...
            if 是著 and 詞長度 == 1 and 前一个詞分詞 and 是動詞(前一个詞分詞):
                print('著應和前面動詞連寫')
                錯誤行數 = 累積字長度 + 字位置 + 1
                錯誤連字符位置 = '前'
                錯誤訊息陣列.append(('E動詞（二）', 錯誤行數, 錯誤連字符位置))
            ...
            # 著應和前面非動詞分寫
            elif 是著 and 前半段的詞分詞 and not 是動詞(前半段的詞分詞):
                print('著應和前面非動詞分寫')
                錯誤行數 = 累積字長度 + 字位置 + 1
                錯誤連字符位置 = '前'
                錯誤訊息陣列.append(('E動詞（二）', 錯誤行數, 錯誤連字符位置))

            # 時貌和後面分寫，跳過不處理 我看著-你 => 我看著你
            
            # 時貌著在句尾是輕聲調 我有看著=>我有看--著
