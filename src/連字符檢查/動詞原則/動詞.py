...
def 是否符合咧原則(句物件):
    ...
    for 詞位置, 詞物件 in enumerate(詞物件陣列):
        ...
        for 字位置, 字物件 in enumerate(詞的字物件陣列):
            ...
            elif 是咧 and not 是咧結尾詞組(詞分詞) and 前半段的詞 and not 是動詞(前半段的詞):
                錯誤行數 = 累積字長度 + 字位置 + 1
                錯誤連字符位置 = '前'
                錯誤訊息陣列.append(('E動詞（二）', 錯誤行數, 錯誤連字符位置))
            ...
            # 咧應和後面分寫 咧-講 => 咧講
            # if 是咧 and 不是咧開頭詞組 and 這個詞的後半段還有字:
            if 是咧 and not 是咧開頭詞組(詞分詞) and (詞長度 - 字位置 - 1 > 0):
                錯誤行數 = 累積字長度 + 字位置 + 1
                錯誤連字符位置 = '後'
                錯誤訊息陣列.append(('E動詞（二）', 錯誤行數, 錯誤連字符位置))
...
def 是否符合著原則(句物件):
    ...
    for 詞位置, 詞物件 in enumerate(詞物件陣列):
        ...
        for 字位置, 字物件 in enumerate(詞的字物件陣列):
            ...
            print('字分詞', 字分詞)
            是著 = (字分詞 in ('著｜tioh8',))
            前半段的詞分詞 = 詞(詞的字物件陣列[:字位置]).轉音(臺灣閩南語羅馬字拼音).看分詞()
            是最後一個字 = (字位置+1 == 詞長度)
            ...
            if 是著:
                ...
                # 著應和前面非動詞分寫
                elif 前半段的詞分詞 and not 是動詞(前半段的詞分詞):
                    print('著應和前面非動詞分寫')
                    錯誤行數 = 累積字長度 + 字位置 + 1
                    錯誤連字符位置 = '前'
                    錯誤訊息陣列.append(('E動詞（二）', 錯誤行數, 錯誤連字符位置))
...
#                 if 是最後一個詞 and 是最後一個字 and 前半段的詞分詞 and not 是動詞(前半段的詞分詞):
#                     print('著應和前面非動詞分寫')
#                     錯誤行數 = 累積字長度 + 字位置 + 1
#                     錯誤連字符位置 = '前'
#                     錯誤訊息陣列.append(('E動詞（二）', 錯誤行數, 錯誤連字符位置))
