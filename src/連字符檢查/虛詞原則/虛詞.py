    ...
    for 詞物件 in 詞物件陣列:
        ...
        for 字位置, 字物件 in enumerate(詞的字物件陣列):
            ...
            if 是否孤字介詞(字分詞) and 詞長度 != 1 and (not 是教典詞組.是教典詞組(詞分詞)):
                # 處理例外情況：西-向
                是可以和前面連寫的介詞 = {'向｜hiong3'}
                # 檢查前面錯誤的連字符
                if 字分詞 not in 是可以和前面連寫的介詞 and 字位置 != 0:
                    錯誤行數 = 累積詞長度 + 字位置 + 1
                    錯誤連字符位置 = '前'
...
def 是否符合結構助詞原則(句物件):
    ...
    for 詞位置, 詞物件 in enumerate(詞物件陣列):
        ...
        for 字位置, 字物件 in enumerate(詞的字物件陣列):
            ...
            # 老--的 輕聲符 合法
            # 遮-的  教典有的詞組 合法
            if (是否結構助詞(字分詞) and 詞長度 != 1 and
                    not (字物件.敢有輕聲標記() or 是教典詞組.是教典詞組(詞分詞))):
                結構助詞位置陣列.append(字位置)
